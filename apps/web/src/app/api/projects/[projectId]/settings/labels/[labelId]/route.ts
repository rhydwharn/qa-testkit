...
export async function PUT(req: NextRequest, { params }: { params: { projectId: string; labelId: string } }) {
...
export async function DELETE(req: NextRequest, { params }: { params: { projectId: string; labelId: string } }) {
  const caller = await requireAuth(req);
  if (!caller) return err("Unauthorized", 401);
